...
public class RobotAiController : MonoBehaviour
{
    ...
    private bool _busy;
    private bool _toggle_object_detection = false;
    private bool _buttonPressed = false;
    private float _lastInferenceTime = 0f;
    private float _inferenceCooldown = 2f; // Minimum time between inference calls
    ...
    private void Update() {
        ...
        // Handle Button Four (Y) with proper state management
        if(OVRInput.GetDown(OVRInput.Button.Four)){
            _toggle_object_detection = !_toggle_object_detection;
            if(_toggle_object_detection){
                _detectionVisualizer.ShowBoundingBoxes = true;
                // Only call inference if enough time has passed since last call
                if(Time.time - _lastInferenceTime >= _inferenceCooldown){
                    Debug.Log("Calling inference");
                    _detector.CallInference();
                    _lastInferenceTime = Time.time;
                }
            } else {
                _detectionVisualizer.ShowBoundingBoxes = false;
            }
        }

        // Continuous inference when toggle is on and button is held (with cooldown)
        if(_toggle_object_detection && OVRInput.Get(OVRInput.Button.Four)){
            if(Time.time - _lastInferenceTime >= _inferenceCooldown){
                _detector.CallInference();
                _lastInferenceTime = Time.time;
            }
